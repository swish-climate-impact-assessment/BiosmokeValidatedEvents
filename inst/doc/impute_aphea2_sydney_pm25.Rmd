...
indir  <- "~/projects_environment_general_local/Air_Pollution_Monitoring_Stations_NSW/OEH_monitor_1995_2014_hrly"
setwd(indir)
dir("data_provided")
...
nsw9498 <-  tbl_df (import (file.path("data_provided", "/OEH (1994-1998)_AllSites_1hrlyData.csv"),
                                      header.at = 3, data.at = 4, date = "Date",
                                      date.format = "%d/%m/%Y"
))
...
#### TODO NOT RUN due to errors

# removing the units
## names(nsw9498)[2:ncol(nsw9498)] <-
#word(colnames(nsw9498)[2:ncol(nsw9498)], start = 1, end = -2)
## Error in stri_sub(string, from = start, to = end) : 
##   argument `to` should be an integer vector (or an object coercible to)
...
# TODO NB is this locations file from the XLS file "Copy of OEH AQMN site list 2014.xls"?
# which sheet?
...
#### TODO check the cause of this warning
## Warning message:
...
# Error: Unknown name
traceback()
## 5: stop("Unknown name", call. = FALSE)
## 4: `$.tbl_df`(mydata, "ws")
## 3: mydata$ws
## 2: checkPrep(mydata, vars, type = "default", remove.calm = FALSE, 
##        strip.white = FALSE)
## 1: timeAverage(nswaq0413, avg.time = "day", data.thresh = 75, interval = "hour")
